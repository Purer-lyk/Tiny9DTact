...
class SensorCalibration:
        ...
        if len(sorted_areas):
            ...
            if self.show_circle_detection:
                ...
                while key != ord('q'):
                    center = (int(x), int(y))
                    radius = int(radius)
                    circle_show = cv2.circle(np.array(diff), center, radius, (0, 0, 255), 1)
    ...
    def mapping_data_collection(self, img, ref):
        ...
        if detect_radius_p:
            ...
            # print(mask.shape)
            # print(mask)
            print(self.sensor.pixel_per_mm)
            ...
            diff_gray = (diff[::, ::, 0] + diff[::, ::, 1] + diff[::, ::, 2]) / 3 # 可能如果画面不是真白，可能会影响
