...
> A compact, Windows-friendly fork of [9DTact](https://linchangyi1.github.io/9DTact/) focused on high-resolution 3D shape reconstruction with a clean, vivid Open3D visualizer.

## About this fork

The upstream 9DTact paper and its Linux/ROS code are the reference. This fork (Tiny9DTact) is the working copy used for iterative software engineering work on Windows:

- **3D Shape Reconstruction** with a redesigned visualization (mesh surface + jet colormap + sidebar with 2D depth map, live readings, and preset views) built on Open3D's `O3DVisualizer` (gui).
- **6D Force Estimation** pipelines kept from upstream; weights live under `saved_models/`.
- **Calibration** scripts for camera intrinsic and the ball-based depth calibration of the gel surface.

## Repository conventions

- Default branch: `main`. Long-lived branches: `main` (stable), `dev` (integration).
- Feature branches follow `feat/<short-topic>`, fixes `fix/<short-topic>`, docs `docs/<short-topic>`, e.g. `feat/visualizer-redesign`.
- All changes land through a pull request. Commits authored under the `ClaudePartner` bot account on behalf of the repository owner.
- Line endings are normalized to LF via `.gitattributes`. Do not commit files with CRLF unless Windows-only.
- See `docs/superpowers/specs/` for design specs (one per feature) and `docs/superpowers/plans/` for implementation plans.
